...
function buildEmailButton(url,label){
    return `
        <div style="margin:28px 0">
            <a href="${url}" style="display:inline-block;background:#ffcc00;color:#061044;padding:14px 22px;border-radius:14px;text-decoration:none;font-weight:900">
                ${label}
            </a>
        </div>
    `;
...
function archeryEmailTemplate(title,message,buttonHtml,small=""){
    return `
        <div style="font-family:Arial,sans-serif;background:#020b3a;padding:28px;color:#fff">
            <div style="max-width:620px;margin:auto;background:#0d1f6b;border:1px solid rgba(255,255,255,.15);border-radius:22px;padding:28px">
                <h1 style="margin:0 0 10px;color:#ffcc00">🎯 ARCHERYSCORE</h1>
                <h2 style="margin:0 0 18px;color:#fff">${title}</h2>
                <p style="font-size:17px;line-height:1.5;color:#c7d2ff">${message}</p>
                ${buttonHtml || ""}
                ${small ? `<p style="font-size:13px;line-height:1.4;color:#9aa7d9">${small}</p>` : ""}
            </div>
        </div>
    `;
}
...
db.serialize(() => {
    ...
    db.run("INSERT OR IGNORE INTO users(id,username,email,password_hash,role,email_verified,blocked) VALUES(1,'admin',?,?,'admin',1,0)",[ADMIN_EMAIL,hashPassword("admin123")], err => {
        ...
        db.run("UPDATE users SET role = 'admin', email_verified = 1, blocked = 0 WHERE username = 'admin'");
    });
        ...
        async function(err){
            ...
            try{
                await sendArcheryMail({
                    ...
                    html:archeryEmailTemplate(
                        "Conferma il tuo account",
                        `Ciao ${username}, grazie per esserti registrato su ArcheryScore. Per attivare il tuo account conferma la tua email.`,
                        buildEmailButton(verifyUrl,"Conferma email"),
                        `Se il pulsante non funziona, copia questo link nel browser: ${verifyUrl}`
                    )
                });
            }catch(mailErr){
            ...
            res.json({
                ...
                dev_verification_link: process.env.NODE_ENV === "production" ? undefined : verifyUrl
            });
        }
    );
});
...
app.get("/api/auth/verify",(req,res)=>{
    ...
    db.get("SELECT id FROM users WHERE verification_token = ?",[token],(err,user)=>{
        if(err || !user){
            return res.status(400).json({ success:false, message:"Link non valido o già utilizzato" });
        }
        ...
        db.run("UPDATE users SET verification_token = ? WHERE id = ?",[verificationToken,user.id],async err=>{
            ...
            try{
                await sendArcheryMail({
                    ...
                    html:archeryEmailTemplate(
                        "Conferma il tuo account",
                        `Ciao ${user.username}, clicca sul pulsante per attivare il tuo account ArcheryScore.`,
                        buildEmailButton(verifyUrl,"Conferma email"),
                        `Se il pulsante non funziona, copia questo link nel browser: ${verifyUrl}`
                    )
                });
    ...
    db.get("SELECT * FROM users WHERE username = ? OR email = ?",[username,username.toLowerCase()],(err,user)=>{
        ...
        if((user.role || "user") !== "admin" && Number(user.email_verified || 0) !== 1){
            return res.status(403).json({
                ...
                message:"Devi confermare la tua email prima di accedere."
            });
        }
        ...
        db.run("INSERT INTO password_resets(token,user_id,expires_at) VALUES(?,?,?)",[token,user.id,expires],async ()=>{
            try{
                await sendArcheryMail({
                    ...
                    html:archeryEmailTemplate(
                        "Reset password",
                        `Ciao ${user.username}, abbiamo ricevuto una richiesta di cambio password. Il link scade tra 30 minuti.`,
                        buildEmailButton(resetUrl,"Reimposta password"),
                        `Se non hai richiesto tu il reset, ignora questa email. Link: ${resetUrl}`
                    )
                });
            }catch(mailErr){
            ...
            res.json({
                success:true,
                message:"Se l'account esiste, riceverai una email per il reset.",
...
app.get("/api/admin/summary", requireAdmin, async (req,res)=>{
    ...
    res.json({
        success:true,
        utenti:Number(utenti?.total || 0),
        verificati:Number(verificati?.total || 0),
        bloccati:Number(bloccati?.total || 0),
        gare:Number(gare?.total || 0),
        allenamenti:Number(allenamenti?.total || 0)
    });
});
...
app.post("/api/admin/users/:id/block", requireAdmin, (req,res)=>{
    const id = Number(req.params.id);
    if(id === req.user.id){
        return res.status(400).json({ success:false, message:"Non puoi bloccare il tuo account" });
    }

    db.run("UPDATE users SET blocked = 1 WHERE id = ?",[id],err=>{
        if(err) return res.status(500).json({ success:false, message:"Errore blocco utente" });
...
app.post("/api/admin/users/:id/unblock", requireAdmin, (req,res)=>{
    db.run("UPDATE users SET blocked = 0 WHERE id = ?",[Number(req.params.id)],err=>{
        if(err) return res.status(500).json({ success:false, message:"Errore sblocco utente" });
        res.json({ success:true });
    });
});

app.post("/api/admin/users/:id/verify", requireAdmin, (req,res)=>{
    db.run("UPDATE users SET email_verified = 1, verification_token = NULL WHERE id = ?",[Number(req.params.id)],err=>{
        if(err) return res.status(500).json({ success:false, message:"Errore verifica utente" });
        res.json({ success:true });
    });
});
...
app.post("/api/admin/users/:id/role", requireAdmin, (req,res)=>{
    ...
    if(id === req.user.id && role !== "admin"){
        return res.status(400).json({ success:false, message:"Non puoi toglierti il ruolo admin" });
    }

    db.run("UPDATE users SET role = ? WHERE id = ?",[role,id],err=>{
        if(err) return res.status(500).json({ success:false, message:"Errore ruolo utente" });
        res.json({ success:true });
    });
});
...
app.delete("/api/admin/users/:id", requireAdmin, (req,res)=>{
    ...
    if(id === req.user.id){
        return res.status(400).json({ success:false, message:"Non puoi eliminare il tuo account admin" });
    }
                    ...
                    db.run("DELETE FROM sessions WHERE user_id = ?",[id],()=>{
                        db.run("DELETE FROM password_resets WHERE user_id = ?",[id],()=>{
                            db.run("DELETE FROM users WHERE id = ?",[id],err=>{
                                if(err) return res.status(500).json({ success:false, message:"Errore eliminazione utente" });
                                res.json({ success:true });
                            });
                        });
                    });
