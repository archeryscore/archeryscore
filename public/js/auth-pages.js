...
async function post(url,data){
    ...
    if(!r.ok && !j.message){
        j.message="Operazione non riuscita";
    }
...
document.getElementById("loginForm")?.addEventListener("submit",async e=>{
    ...
    if(!j.success){
        if(j.needs_verification){
            return showMsg("Conferma la tua email prima di accedere. Controlla la posta o rimanda la conferma dalla registrazione.");
        }
...
document.getElementById("registerForm")?.addEventListener("submit",async e=>{
    ...
    if(!j.success){
        return showMsg(j.message||"Registrazione non riuscita");
    }
    ...
    showMsg(j.message||"Account creato. Controlla la mail per confermare.","success");

    if(j.dev_verification_link){
        showMsg("Modalità locale: apri questo link per confermare: "+j.dev_verification_link,"success");
    }
});
...
document.getElementById("resendVerificationForm")?.addEventListener("submit",async e=>{
    ...
    if(!j.success){
        return showMsg(j.message||"Invio non riuscito");
    }
...
document.getElementById("requestResetForm")?.addEventListener("submit",async e=>{
    ...
    if(!j.success){
        return showMsg(j.message||"Richiesta non riuscita");
    }

    showMsg(j.message||"Se l'account esiste, riceverai una email per il reset.","success");

    if(j.dev_reset_link){
        showMsg("Modalità locale: apri questo link per resettare: "+j.dev_reset_link,"success");
    }
});
...
if(resetToken && document.getElementById("resetForm")){
    ...
    requestForm?.classList.add("hidden-auth-box");
...
document.getElementById("resetForm")?.addEventListener("submit",async e=>{
    ...
    const f=new FormData(e.target);
    const j=await post("/api/password/reset",{
        token:f.get("token"),
        password:f.get("password")
    });

    if(!j.success){
        return showMsg(j.message||"Reset non riuscito");
    }
...
if(location.pathname.endsWith("/verify-email.html")){
    ...
    if(!token){
        ...
        fetch("/api/auth/verify?token="+encodeURIComponent(token))
            ...
            .then(j=>{
                if(!j.success){
                    showMsg(j.message||"Conferma non riuscita");
                    return;
                }

                document.getElementById("verifyText").textContent="Email confermata correttamente.";
