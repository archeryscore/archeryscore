...
function showAdminConfirm({title="Conferma",message="",confirmText="Conferma",danger=false}){
    return new Promise(resolve=>{
        document.querySelector(".admin-modal-backdrop")?.remove();
        const wrap=document.createElement("div");
        wrap.className="admin-modal-backdrop";
        wrap.innerHTML=`<div class="admin-modal"><h3>${esc(title)}</h3><p>${esc(message)}</p><div class="admin-modal-actions"><button class="admin-modal-cancel">Annulla</button><button class="admin-modal-confirm" style="${danger?"background:#ff5c7a;color:white":""}">${esc(confirmText)}</button></div></div>`;
        document.body.appendChild(wrap);
        wrap.querySelector(".admin-modal-cancel").onclick=()=>{wrap.remove();resolve(false);};
        wrap.querySelector(".admin-modal-confirm").onclick=()=>{wrap.remove();resolve(true);};
    });
}
...
function renderUsers(users){
    document.getElementById("adminUsers").innerHTML=users.map(u=>`<tr><td><strong>${esc(u.username)}</strong><br><small>ID ${esc(u.id)}</small></td><td>${esc(u.email)}</td><td><span class="admin-badge">${esc(u.role||"user")}</span></td><td>${Number(u.email_verified||0)===1?'<span class="admin-badge ok">Sì</span>':'<span class="admin-badge no">No</span>'}</td><td>${Number(u.blocked||0)===1?'<span class="admin-badge no">Bloccato</span>':'<span class="admin-badge ok">Attivo</span>'}</td><td>${esc(u.gare_count||0)}</td><td>${esc(u.allenamenti_count||0)}</td><td class="admin-date">${esc(formatDateIT(u.last_login))}</td><td><div class="admin-actions"><button class="admin-action small blue" onclick="viewUser(${u.id})">Vedi</button>${Number(u.email_verified||0)!==1?`<button class="admin-action small green" onclick="verifyUser(${u.id},'${esc(u.username)}')">Verifica</button>`:""}${Number(u.blocked||0)===1?`<button class="admin-action small green" onclick="unblockUser(${u.id},'${esc(u.username)}')">Sblocca</button>`:`<button class="admin-action small gray" onclick="blockUser(${u.id},'${esc(u.username)}')">Blocca</button>`}<button class="admin-action small orange" onclick="setRole(${u.id},'${u.role==='admin'?'user':'admin'}','${esc(u.username)}')">${u.role==='admin'?'Utente':'Admin'}</button><button class="admin-action small red" onclick="deleteUser(${u.id},'${esc(u.username)}')">Elimina</button></div></td></tr>`).join("");
}
