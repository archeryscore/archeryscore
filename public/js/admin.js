...
async function sendReset(id,name){if(!await showAdminConfirm({title:"Inviare reset password?",message:`Inviare una mail di reset password a ${name}?`,confirmText:"Invia mail"}))return;await api(`/api/admin/users/${id}/reset-password`,{method:"POST"});await showAdminConfirm({title:"Reset inviato",message:"Se SMTP è configurato, l'utente riceverà la mail.",confirmText:"OK"});}
